...
import { BlockList } from '~/components/Analytics';
...
const DisplayMessage = ({ text, isCreatedByUser, message, showCursor }: TDisplayProps) => {
  ...
  const analyticsBlocks = useMemo(() => {
    ...
    if (
      ...
      Array.isArray((analytics as { blocks?: unknown }).blocks)
    ) {
      return (analytics as { blocks: AnalyticsBlock[] }).blocks;
    }
    return null;
  }, [message?.metadata]);

  const hasAnalyticsBlocks =
    !isCreatedByUser && analyticsBlocks != null && analyticsBlocks.length > 0;

  const content = useMemo(() => {
    if (hasAnalyticsBlocks) {
      return null;
    }
    if (!isCreatedByUser) {
    ...
    return <>{text}</>;
  }, [hasAnalyticsBlocks, isCreatedByUser, enableUserMsgMarkdown, text, isLatestMessage]);

  // Keep analytics blocks out of `.markdown` — those styles restyle pivot <table>
  // into a second rounded card (double container). Reports render BlockList bare.
  if (hasAnalyticsBlocks && analyticsBlocks) {
    return (
      <Container message={message}>
        <BlockList blocks={analyticsBlocks} isLatestMessage={isLatestMessage} />
      </Container>
    );
  }
