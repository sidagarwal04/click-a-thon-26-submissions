...
- A handoff containing only `run_id` and `investigation_id` is valid. Use
  `run_analytics` with `action: list_run`, then retrieve only the canonical report,
  insight, chart, schema, and review artifacts required for delivery. Do not retrieve
  aggregate CSV bodies unless a specific displayed result cannot be verified by a
  bounded ClickHouse query.
- Query ClickHouse directly for every metric that will appear in the executive
  summary, insight cards, actions, or HyperDX tiles. Restrict every artifact read to
  the supplied run ID and preserve the ClickHouse query IDs. Analytics proposes the
  evidence; this Finalizer verification is the publication gate.
- Create the HyperDX dashboard specification from verified Analytics chart artifacts.
  A chart may be published only when its source is the run's live ClickHouse table,
  its metric definition matches the artifact, and the HyperDX tool returns a stable
  dashboard/chart ID and resolving URL. Never substitute a portal-native chart.
...
- When `publish_finalizer_response` is enabled, publish the validated canonical envelope through that tool using the investigation ID, then return its result ID. This is the Input Page delivery boundary.
- In this isolated Analytics chain, recover the investigation ID only from the
  generated base-table prefix `default.inv_<32 lowercase hex>_...` in the supplied
  DDL and format that value as a UUID. Do not require the pre-Analytics
  instrumentation conversation or add another input field.
