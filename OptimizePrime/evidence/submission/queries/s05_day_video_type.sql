-- DAY grain · video_type filter · session tier.
-- video_type is NOT a cube level of cc_hour_agg: it rides off content_id via content_dim.
...
(
    SELECT if(c.has_catalog = 0, '(unknown)', if(c.video_type = '', '(blank)', c.video_type)) AS attr,
    ...
    LEFT ANY JOIN
    (
        SELECT content_id, video_type, toUInt8(1) AS has_catalog FROM content_dim FINAL
