-- DAY grain · show_name filter · session tier.
-- show_name is NOT a cube level of cc_hour_agg: it rides off content_id via content_dim.
...
(
    SELECT if(c.has_catalog = 0, '(unknown)', if(c.show_name = '', '(blank)', c.show_name)) AS attr,
    ...
    LEFT ANY JOIN
    (
        SELECT content_id, show_name, toUInt8(1) AS has_catalog FROM content_dim FINAL
