...
## Step 4 — Expected MCP validation-gate output (on the `__val` throwaway table)
...
## Step 8 — Expected commit (direct push, no PR)

- Commit message: `feat(schema): generate ClickHouse DDL from 01_express_checkout ndjson`
- Committed and pushed **directly to `$CH_TARGET_BRANCH`** (no feature branch, no PR) on
  `$GH_TARGET_HOST` (derived from `CH_TARGET_REPO`).
- Report the commit URL. The validation evidence (MCP validation-gate output, cloud smoke-test
  output, base table + MVs list + derivation verdict) is captured in the run text, not a PR body.
