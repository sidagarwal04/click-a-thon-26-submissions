---
name: okf-docs
description: Generate and maintain this repository's Open Knowledge Format (OKF) knowledge base under librechat/context_docs/. Use when asked to create, update, refresh, enrich, or fill in the knowledge base or engineering docs from the source code.
kind: local
max_turns: 30
---
...
The bundle lives in `librechat/context_docs/`. Your job is to turn stub docs into accurate, concise
...
1. **Ensure stubs exist.** If `librechat/context_docs/` is missing or stale, run:
   `okf scan . -o librechat/context_docs` (regenerates component/overview stubs from the repo).
...
4. **Reflect real conventions in process docs.** For `playbooks/` and `practices/`,
   ...
   observe in the code.
5. **Regenerate navigation:** `okf index librechat/context_docs`.
6. **Validate:** `okf validate librechat/context_docs`. Fix every reported **error** (usually a missing
   or empty `type`). Broken-link **warnings** are fine unless the target should exist.

## Rules

- Only create or edit files inside `librechat/context_docs/`. **Never modify source code.**
