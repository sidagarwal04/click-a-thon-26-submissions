...
**→ Full results, latencies and the reproducible `query_log` trace: [`docs/BENCHMARK.md`](docs/BENCHMARK.md)**;
tagged queries in [`sql/05_benchmark.sql`](sql/05_benchmark.sql).

**Serving layer — three tables, each sorted for its own query pattern:**
`dim_minute (dim, value, minute)` for the total + any single-dim filter (one keyed granule),
`concurrency_1m` dims-first for multi-dim / content-attribute combos (leading-dim prefix prune),
and `user_minute` for user-grain concurrency (distinct users, set-based). Built from the
`hist_minute_full` base in [`sql/04_serving.sql`](sql/04_serving.sql).
