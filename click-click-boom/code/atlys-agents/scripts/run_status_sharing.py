"""Fourth real end-to-end pipeline run: Visa Status Sharing — a viral acquisition
loop (share -> channel select -> link generated -> recipient opens -> recipient
CTA). Structurally distinct from prior specs: sharer events carry the full
envelope, but recipient events (link_opened, recipient_cta_clicked) are keyed by
share_id, not application_id/user_id -- a genuinely different join key shape."""
import json
import pathlib
import sys
...
SPEC_DIR = pathlib.Path(
    "/Users/anshmehta/Downloads/Clickhouse Hackathon/click-a-thon-2026-main/Atlys/specs/03_status_sharing"
)
PER_EVENT_SAMPLE = 30
