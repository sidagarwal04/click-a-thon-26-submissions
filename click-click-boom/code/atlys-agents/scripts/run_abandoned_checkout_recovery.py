"""Third real end-to-end pipeline run: Abandoned Checkout Recovery — a nudge/
re-engagement funnel (abandon -> reminder -> open/click -> resume -> reconvert),
distinct from Instant Forex in that its PM questions (recovery rate by drop_step,
channel funnel, timing) are naturally MV-shaped from day one."""
import json
import pathlib
import sys
...
SPEC_DIR = pathlib.Path(
    "/Users/anshmehta/Downloads/Clickhouse Hackathon/click-a-thon-2026-main/Atlys/specs/04_abandoned_checkout_recovery"
)
PER_EVENT_SAMPLE = 30
...
def main():
    ...
    result = ingest_spec(
        spec_name="abandoned_checkout_recovery",
